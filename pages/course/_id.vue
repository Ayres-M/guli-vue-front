  ...
  <div id="aCoursesList" class="bg-fa of">
    ...
    <section class="container">
      <section class="path-wrap txtOf hLh30">
        ...
        \
        <span class="c-333 fsize14">{{ courseWebVo.subjectLevelTwoId }}</span>
      </section>
      <div>
          ...
          <section class="ml20 mr15">
            ...
            <section class="c-attr-mt" v-if="isbuy || Number(courseWebVo.price)===0">
              <a
                ...
                >立即观看</a
              >
            </section><section class="c-attr-mt" v-else>
              <a
                @click="createOrders()"
                href="#"
                title="立即购买"
...
export default {
  ...
  //   //使用params.id获取路径中的id
  //   return courseApi.getCourseInfo(params.id).then((resp) => {
  //     return {
  //       courseWebVo: resp.data.data.courseWebVo,
  //       chapterVideoList: resp.data.data.chapterVideoList,
  //     };
  //   });
  // },
  data() {
  ...
  methods: {
    //生成订单
    createOrders() {
      ordersApi.createOrders(this.courseId).then((resp) => {
        //获取返回的订单号
        //跳转到订单显示页面
        this.$router.push({ path: "/orders/" + resp.data.data.orderId });
      });
    },
    //展示课程
    showCourseInfo() {
    ...
    showComment() {
      commentApi
        .getCommentPage(this.courseId, this.page, this.limit)
        .then((resp) => {
    ...
    addComment() {
      ...
      commentApi.addComment(this.comment).then((resp) => {
    ...
    gotoPage(page) {
      commentApi
        .getCommentPage(this.courseId, page, this.limit)
        .then((resp) => {
          this.data = resp.data.data;
        });
    },
